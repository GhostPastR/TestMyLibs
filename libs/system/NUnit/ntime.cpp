...
NTime NTime::time( const NDistance &distance, const NSpeed &speed ) noexcept
{
    return NTime( ( distance.m() / speed.ms() ).toUInt32() );
}

NTime &NTime::setTime(const NDistance &distance, const NSpeed &speed) noexcept
{
    *this = NTime::time( distance, speed );
    return *this;
}

QString NTime::toString(const QString &format ) noexcept
{
    if( format.isEmpty() )
       {
